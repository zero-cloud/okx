...
def set_leverage(leverage):
    ...
    if result["code"] == "0":
        print(f"[{now_str()}] long 杠杆已设置为 {leverage} 倍")
    else:
        print(f"[{now_str()}] long 设置杠杆失败: {result}")
...
def validate_long_stop_loss_price(entry_price, stop_loss_price):
    if stop_loss_price >= entry_price:
        print(f"[{now_str()}] 多单止损价 {stop_loss_price:.2f} 必须低于挂单价 {entry_price:.2f}")
        return False
    return True


def validate_long_take_profit_price(entry_price, take_profit_price):
    if take_profit_price <= entry_price:
        print(f"[{now_str()}] 多单止盈价 {take_profit_price:.2f} 必须高于挂单价 {entry_price:.2f}")
        return False
    return True
...
def place_entry_limit_order(entry_price, quantity, stop_loss_price=None, take_profit_price=None):
    ...
    if LOCAL_SIMULATION:
        ...
        print(f"[{now_str()}] 模拟挂限价开多单: price={entry_price}, qty={quantity}")
    ...
    if result["code"] == "0":
        ...
        print(f"[{now_str()}] 限价挂多单成功: ordId={ord_id}, price={entry_price}, qty={quantity}")
        return ord_id

    print(f"[{now_str()}] 限价挂多单失败: {result}")
    return None


def place_reduce_order(quantity, reduce_only=True):
    if LOCAL_SIMULATION:
        print(f"[{now_str()}] 模拟平多: qty={quantity}")
    ...
    result = response.json()
    if result["code"] == "0":
        print(f"[{now_str()}] 平多成功: qty={quantity}")
        return result["data"][0]["ordId"]

    print(f"[{now_str()}] 平多失败: {result}")
    return None
...
def get_pending_long_limit_orders():
    global simulated_entry_order
...
def trading_strategy(
    ...
    print(f"[{now_str()}] BTC 实盘限价挂多单程序启动")
    print(
    ...
    while True:
        try:
            current_price = get_btc_price()
            print(f"[{now_str()}] 当前 BTC 价格 = {current_price:.2f}")
            ...
            if position:
                print(f"[{now_str()}] 当前持仓: side=long, qty={position['pos']}, avgPx={position['avgPx']}")
            ...
            pending_entries = get_pending_long_limit_orders()
            ...
            for i in range(n):
                ...
                if not validate_long_stop_loss_price(entry_price, stop_loss_price):
                    continue
                if not validate_long_take_profit_price(entry_price, take_profit_price):
                    continue
            ...
            print(f"[{now_str()}] 本轮共挂出 {placed_count}/{len(planned_orders)} 个阶梯限价多单")
...
def main():
    ...
    n = 4
    #第一次挂单价
    first_entry_price = 75000
    #价格步长
    entry_price_step = 200
    #第一次止盈价
    first_take_profit_price = 80000
    #止盈步长
    take_profit_step = 0
    #第一次止损价
    first_stop_loss_price = 70000
    #止损步长
    stop_loss_step = 0
    #杠杆
    leverage = 100
